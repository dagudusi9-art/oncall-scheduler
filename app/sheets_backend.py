...
def _col_letter(n: int) -> str:
    """1始まりの列番号をA1表記の列文字に変換する(26列を超えても対応)。"""
    letters = ""
    while n > 0:
        n, rem = divmod(n - 1, 26)
...
# ----------------------------------------------------------------------
# キー指定の1行だけを安全に更新するシート (member×year_month のような
# 「1行=1レコード」形式のシート用。ws.clear()は一切使わない)
# ----------------------------------------------------------------------
...
def upsert_keyed_row(
    ...
    """key_cols(例: ["member_name", "year_month"])の値がkey_valuesと一致する
    行を1件だけ特定し、その行の範囲だけを更新する。一致する行が無ければ
    末尾に1行追記する。

    この関数はシート全体の ws.clear() を絶対に使わない。読み込み時点の
    スナップショットは行の位置(=対象行番号)を特定するためだけに使い、
    書き込みは対象行(またはappendによる新規行)のセル範囲にのみ及ぶため、
    他の行(=他メンバー・他月のデータ)には一切触れない。

    並行実行時の仕様(last-write-wins):
    - 異なるkey(例: 別メンバー、または同メンバーでも別年月)を対象とする
      呼び出し同士は、read/writeがどのようにインターリーブしても
      互いのデータを破壊しない(対象行が異なるため)。
    - 同一key(=同一member_name×year_month)を2セッションがほぼ同時に
      更新した場合、既にその行が存在していれば、最後に ws.update() を
      実行したセッションの内容がそのまま残る(last-write-wins)。
      先に書き込んだセッションの内容は後発の書き込みで上書きされ、
      2つの内容がマージされることはない。
    - [既知の制約] 対象行がまだ一度も作成されていない状態(=そのkeyの
      初回保存)で、2セッションがほぼ同時に「該当行なし」と判定し、
      それぞれ ws.append_row() を実行した場合、理論上は同一keyの行が
      2行できる可能性がある(いずれかの1医師が同じ月を複数タブ・複数端末
      からほぼ同時に初回保存した場合のみ起こりうる、極めて稀なケース)。
      この場合もシート全体の破壊や他レコードへの影響は無く、読み込み側
      (例: data_store.py の _fetch_v2_month_records)はシート内で最後に
      出現する行の内容を採用するため、実害としてはlast-write-winsと
      同等になる(重複行は残るが、次にどちらかのセッションが同じkeyで
      再度保存すれば、先に見つかった方の行が更新される)。
    ...
    if sh is None:
        return False
    try:
        ws = _get_or_create_ws(sh, sheet_name, rows=rows, cols=cols)
        ...
        if existing_header != header:
            # ヘッダー行のみを補正する(データ行には触れない)
            ws.update(values=[header], range_name="A1")
            existing_header = header

        col_index = {name: i for i, name in enumerate(existing_header)}
        row_values = [_to_row_cell(row_dict.get(col)) for col in header]

        target_row_number = None
        for i, row in enumerate(values[1:], start=2):  # 1-indexed、ヘッダーの次から
            is_match = True
            for kc in key_cols:
                idx = col_index.get(kc)
                cell = row[idx] if (idx is not None and idx < len(row)) else ""
                if str(cell) != str(key_values.get(kc, "")):
                    is_match = False
                    break
            if is_match:
                target_row_number = i
        ...
        if target_row_number is not None:
            ...
            ws.update(
                values=[row_values],
                range_name=f"A{target_row_number}:{end_col}{target_row_number}",
            )
        else:
...
def append_row_safe(
    ...
    """末尾に1行だけ追記する(append-only)。既存行には一切触れないため、
    履歴シートなど「追記のみで良い」用途に使う。ws.clear()は使わない。"""
    sh = get_spreadsheet()
    if sh is None:
        return False
    try:
